...
public class UserAssertionTest {
  ...
  // 1. Verify the if the total record is 20
    ...
    @Test
    public void test002(){
        response.body("[1].name", equalTo("Trilochana Chattopadhyay"));
    }

    //3. Check the single ‘Name’ in the Array list (Subhashini Talwar)
    @Test
    public void test003(){
        response.body("name",hasItem("Laxman Desai"));
    }
    ...
    @Test
    public void test004(){
        response.body("name", hasItems("Ms. Deeptimoy Varrier","Kirti Chaturvedi","Deependra Verma"));
    }
    //5. Verify the emai of userid = 5471 is equal “adiga_aanjaneya_rep@jast.org”
    @Test
    public void test005(){
        response.body("[4].email", equalTo("verma_deependra@goyette.example"));
    }
